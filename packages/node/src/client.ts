...
export interface NodeClient<Routes extends {
  server: {
...
class WebSocketClient<Routes extends {
  ...
  private clientApi: NodeClient<Routes, Context>;
    ...
    return {
      ...
      async return(): Promise<IteratorResult<T, void>> {
        if (!done && self.ws && self.ws.readyState === WebSocket.OPEN) {
          // Reuse message object for cancellation
          self.sendMessage({
            id: messageId,
            type: 'stream-cancel'
          });
  ...
  private createClientApi(): NodeClient<Routes, Context> {
    const self = this;
    ...
    return api as NodeClient<Routes, Context>;
  }

  public getClientApi(): NodeClient<Routes, Context> {
